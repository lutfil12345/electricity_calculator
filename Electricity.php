...
function powercalculation($volt, $ampere) {
    return ($volt * $ampere) / 1000; 
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST'):
    ...
    $power = powercalculation($voltage, $ampere);
    $rate = rateRM($rate_sen);
?>

<div class="mt-5">
    <h4>Results</h4>
    <p class="text-primary"><strong>Power:</strong> <?= number_format($power, 5) ?> kW</p>
    <p class="text-primary"><strong>Rate:</strong> <?= number_format($rate, 3) ?> RM</p>
</div>
